...
管理员用例图的绘制结果见图3-5。管理员登录进入本系统操作的功能包括增删改查装修材料，工程信息，材料分类信息，管理材料订单以及工程订单，管理用户。

![](/md/blog.005.png)

图3-5 管理员用例图

用户用例图的绘制结果见图3-6。用户登录进入本系统操作的功能包括购买装修材料，支付装修材料订单，对工程下单，审核管理员对工程订单提供的报价信息，支付工程订单。
...
（2）使用Visio这样的常用的实体属性图绘制工具来绘制装修材料实体属性图，绘制结果见图4-4。
...
（4）使用Visio这样的常用的实体属性图绘制工具来绘制工程实体属性图，绘制结果见图4-6。
...
通过上节内容可以知晓数据库中的各个实体，并通过一定方式把这些实体表示的内容进行数据表的转换，通常来说，每个实体都会对应一张具体的数据表，在本系统指定的数据库中创建命名好的数据库，才可以对数据表进行创建与设计。小工程预算系统数据表设计结果展示如下：
...
|cailiaofenlei|材料分类|varchar(200)|否|
表4-2 订单信息表
...
|cailiaomingcheng|材料名称|varchar(200)|否|
|cailiaoleixing|材料类型|varchar(200)|否|
|jiage|价格|float|否|
...
|addtime|创建时间|timestamp|否|
|dingdanbianhao|订单编号|varchar(200)|是|
|gongchengmingcheng|工程名称|varchar(200)|否|
|mianji|面积|varchar(200)|否|
|baojia|报价|float|是|
|xiadanshijian|下单时间|datetime|是|
|yonghuming|用户名|varchar(200)|是|
|xingming|姓名|varchar(200)|是|
...
|gongchengmianji|工程面积|varchar(200)|否|
|zhuangxiujiancai|装修建材|longtext|是|
|gongchengjieshao|工程介绍|longtext|是|
表4-7 公告资讯表
...
|id (主键)|主键|bigint(20)|否|
|addtime|创建时间|timestamp|否|
|cailiaobianhao|材料编号|varchar(200)|否|
|cailiaomingcheng|材料名称|varchar(200)|否|
|cailiaofenlei|材料分类|varchar(200)|否|
|tupian|图片|varchar(200)|是|
|jiage|价格|float|否|
...
|cailiaojieshao|材料介绍|longtext|是|
